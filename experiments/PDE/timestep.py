...
for i,s in enumerate(steps):
    print(s)
    u=PDE_solve(N=N0*s,dof=100,points=points)
    for j,p in enumerate(points):
        res[i,j]=u(p[0],p[1])
    
fig,ax=plt.subplots()
for j,p in enumerate(points):
    ax.loglog(1/(N0*steps[:-1]),np.abs(res[:-1,j]-res[-1,j]),lw=2,label="$\mathbf{X}_0=(" + str(p[0]) + ", " + str(p[1]) + ")$",marker=markers[j],markersize=10)
ax.legend()
