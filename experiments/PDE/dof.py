...
for i,s in enumerate(steps):
    print(s)
    u=PDE_solve(N=N,dof=s,points=points)
    dof[i] = len(u.vector())
...
fig,ax=plt.subplots()
for j,p in enumerate(points):
    ax.loglog(dof[:-1],np.abs(res[:-1,j]-res[-1,j]),lw=2,label="$\mathbf{X}_0=(" + str(p[0]) + ", " + str(p[1]) + ")$",marker=markers[j],markersize=10)
ax.legend()
