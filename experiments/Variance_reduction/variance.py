...
pre_run = pde_engine.solve(N=N,resolution=resolution,cutoff=cutoff,points=points,all_timesteps=True, return_grad=False)

ref = pre_run[-1][0](init[0],init[1])
for i,e in enumerate(engines):
    print(e)
    results[i,:] = e.solve(init=init,N=N,reps=reps,resolution=resolution,pre_run=pre_run)
